...
class Stock(Base):
    ...
    fwd_pe_history = relationship("FwdPEHistory", back_populates="stock", cascade="all, delete-orphan")
...
class FwdPEHistory(Base):
    """
    Daily snapshots of forward PE so we can derive the 3-year rolling mean.
    Only stored for watchlisted tickers to keep the DB lean.
    """
    __tablename__ = "fwd_pe_history"

    id = Column(Integer, primary_key=True, index=True)
    stock_id = Column(Integer, ForeignKey("stocks.id", ondelete="CASCADE"), nullable=False)
    captured_on = Column(DateTime, default=datetime.utcnow, index=True)
    forward_pe = Column(Float, nullable=False)

    stock = relationship("Stock", back_populates="fwd_pe_history")

    __table_args__ = (
        Index("ix_fwdpe_stock_date", "stock_id", "captured_on"),
    )
